...
.upload-box { border: 2px dashed #ccc; padding: 30px; text-align: center; border-radius: 10px; background-color: white; }
.stButton button { background-color: #ff4b4b; color: white; border-radius: 8px; height: 50px; width: 100%; font-size: 16px; }
.processing { font-size: 18px; color: #ff4b4b; text-align: center; margin-top: 10px; }
</style>
...
if convert and uploaded_file:
    st.markdown("""
    <div class="processing" id="processing-msg">Processing your file... ⏳</div>
    <script>
        const msg = document.getElementById("processing-msg");
        let dots = 0;
        setInterval(() => {
            dots = (dots + 1) % 4;
            msg.innerHTML = "Processing your file" + ".".repeat(dots) + " ⏳";
        }, 500);
    </script>
    """, unsafe_allow_html=True)
